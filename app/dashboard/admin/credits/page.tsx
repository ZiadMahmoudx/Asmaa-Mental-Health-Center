...
export const metadata: Metadata = {
  title: "سجل الأمان المالي وأرصدة المرضى | لوحة الإدارة",
  description: "متابعة ديون ومستحقات المرضى، تسوية المبالغ عبر InstaPay، وتدقيق العمليات المالية.",
};

export const dynamic = "force-dynamic";

export default async function AdminCreditsPage() {
  await requireRolePage(["ADMIN"], "/dashboard/admin/credits");
    ...
    return (
      <div className="min-h-screen py-16 bg-slate-50">
        <div className="max-w-lg mx-auto px-4 text-center space-y-3">
          ...
          <h1 className="text-lg font-black text-red-700">تعذّر تحميل سجل أرصدة المرضى</h1>
          <p className="text-xs text-slate-600">{creditsResult.messageAr}</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen py-8 bg-slate-50 text-slate-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        ...
        <div className="flex items-center gap-2 text-xs font-semibold text-slate-500">
          ...
          <span>/</span>
          <span className="text-slate-900 font-bold">سجل الأمان المالي وأرصدة المرضى</span>
        </div>
